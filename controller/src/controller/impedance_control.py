...
class ImpedanceControl(Node):
    ...
    def ImpedanceControl(self):
        ...
        imp_torque = self.B * (self.velocity - self.velocity_e) + self.K * (self.position - self.position_e) # At Ankle
        imp_torque_msg = ImpedanceTorque()
        imp_torque_msg.tau_imp = imp_torque/9  # Change 9 to something
